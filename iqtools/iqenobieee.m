function ENOB = iqenobieee(visa_addr_awg, visa_addr_dca, tones, fsAWG, awg_channels, awg_trig, ...
    scopechannels, scopeRST, autoScopeAmpl, scopeAmpl, scopeAvg, analysisAvg, ...
...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    % AWG
    amplitude = 0.5;   % the output amplitude in V
 ...
 %% MEASUREMENT SECTION
    ...
    % force balanced to 1 if using the DCA's differential channel
...
%         % ---
        ...
        for nn = 1:length(tones)
            ...
            binblockwrite(awg,samples,'int8',sprintf(':TRAC%d:DATA 1,0,',awg_meas_channel));
            fprintf(awg, '\n'); % terminate the write operation
            fprintf(awg, sprintf('VOLT%d 0.5',awg_meas_channel));
            fprintf(awg, sprintf('OUTP%d ON',awg_meas_channel));
...
%             fprintf(dca, ':WAV:SOUR FUNC1');
            ...
            fsDCA = 1/(10*(length(samples)/(10*fsAWG))/(length(samples)*spb));
            faxis = fsDCA*(-length(ydata)/2:length(ydata)/2-1)/length(ydata);
            spectrum = fftshift(fft(ydata)/length(ydata));
            spectrum(abs(faxis)>fsAWG/2) = 1e-15;
            ...
            pguess = [1e-6,1,1e-3]; % initial guess
    ...
    %              if (length(xdata) > 250)
    %                  xlim(axesHandles(1), [xdata(1) xdata(250)]);
    %              end
    ...
    %             % compute the SINAD according to IEEE 1658-2011 p. 52 eq. (30)
    %             SINAD = (A/sqrt(2))/sqrt(1/length(ydata)*fun2(p));
    %             SINAD_dB(nn) = 20*log10(SINAD);
    %             % ---
    % 
    %             % compute the ENOB according to IEEE 1658-2011 p. 58 eq. (43)
    %             V = amplitude;
    %             ENOB(nn) = SINAD_dB(nn)/20*log2(10)-0.5*log2(1.5)-log2(A/V);
    % %            fprintf('Ftone: %.3f GHz, ENOB: %.3f\n',ft(nn)*1e-9,ENOB(nn));
    %             % ---

                % set the the FSR to the amplitude that we measured at the first
                % frequency point (lowest frequency)
                if FSR_set == 0
                    FSR = 2*A;
                    FSR_set = 1;
                end
...
%     figure
%     set(gcf,'Color','white');
%     plot(tones*1e-9,ENOB,'b','Linewidth',2,'Marker','d','MarkerFaceColor','b')
%     box on
%     grid on
%     xlim([0 46]);
%     ylabel('ENOB')
%     xlabel('Frequency [ GHz ]');
%     title({'M8196A ENOB',sprintf('%.2f GSa/s, %.2f V Amplitude',fsAWG*1e-9,amplitude)})
    % ---

    % assign main result variables to base workspace
%     assignin('base','SINAD_dB',SINAD_dB');
...
function [samples, ft, tdiv] = ...
    ...
    % - tdiv    = Timebase setting for the DCA-J in seconds
	...
	% uses the full DAC code range, 
    % and report back the final used frequency.
	J  = Closest_Prime(J);
	ft = fupdate*J/M;
    ...
    % Compute the timebase setting for the DCA to fullfil eq. (12) of the
    % standard; the factor 10 comes from the DCA's ten divisions.
    tdiv = dca_mem_depth/(10*2*fupdate);
