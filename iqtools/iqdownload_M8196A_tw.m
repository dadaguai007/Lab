function result = iqdownload_M8196A_tw(arbConfig, fs, data, marker1, marker2, segmNum, channelMapping, run)
...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    f = instrfind('Type', 'visa-generic', 'RsrcName', RsrcName, 'Tag', '');
    if isempty(f)
        f = visa(obj.VISA_Vendor, RsrcName);
    else
        fclose(f);
        f = f(1);
    end
...
% download an arbitrary waveform signal to a given channel and segment
    ...
    if (isfield(arbConfig, 'M8196Acorrection') && arbConfig.M8196Acorrection ~= 0)
        a = query(f, sprintf(':CHAR%d?', chan));
...
%        end
        ...
        while (offset < segm_len)
            if (use_binblockwrite)
                ...
                binblockwrite(f, data(1+offset:offset+len), 'int8', cmd);
            else
