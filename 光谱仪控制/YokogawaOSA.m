classdef YokogawaOSA < Device
    ...
    methods
        ...
        function [wavelength,waveform] = GetOSATrace(obj, traceid)
            ...
            get_txt = query(g,sprintf(':TRAC:X? TR%s,%d,%d',traceid,1,n_samp));
            x = textscan(get_txt,'%f','delimiter',',');
            wavelength = x{1};
            % get the waveform (power) data, n_avg iterations are averaged
            get_txt = query(g,sprintf(':TRAC:Y? TR%s,%d,%d',traceid,1,n_samp));
        ...
        function pow = ReadPower(obj)
            obj.Single();
            g = obj.Init();
            fopen(g);
            obj.Authentication(g);
            % perform power analysis
             fprintf(g,':CALCulate:CATegory POWer');
             fprintf(g,':CALCulate:IMMediate');
             pow = str2double(query(g,':CALCulate:DATA?'));
            fclose(g);
        end
