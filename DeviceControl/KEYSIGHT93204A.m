classdef KEYSIGHT93204A < Device
    ...
    methods
        function obj = KEYSIGHT93204A(IPaddr)
            if nargin < 1
                obj.Addr = '192.168.1.99';
            else
                obj.Addr = IPaddr;
            end
...
%             obj.ConnectionType = 'usb';
        ...
        function y = readwaveform(obj,chIDs)
            ...
            %%
            set(g, 'Timeout', 5);
            set(g, 'InputBufferSize', 1e8);    %when ASCii type
            %
            fopen(g);
            % data acqusition
            % setting
...
% 判断是否会超阈值
        ...
        function AutoCenterDisplay(obj,chID,time,~)
            ...
            if flagAutoSetVerticalScale
                ...
                fprintf(g,cmd);
                cmd = sprintf(':CHANnel%d:BWLimit %d',chID,1);
                fprintf(g,cmd);
                % adjust best
                cmd = sprintf(':CHANnel%d:SCALe?',chID);
