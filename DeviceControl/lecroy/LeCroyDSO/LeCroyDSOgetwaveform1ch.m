function [Y, T, DSO]=LeCroyDSOgetwaveform1ch(deviceObj, appObj, ch_no, samplerate, memory)
% Tianwai 08/27/2018 *to remove "T" *to remove the control sentence
...
set(deviceObj.Channel(ch_no), 'State', 'on');   

groupObj = get(deviceObj, 'Util');
groupObj = groupObj(1);

invoke(groupObj, 'sendcommand', sprintf('VBS app.Acquisition.C%d.View=True',ch_no));
...
set(deviceObj.Channel(ch_no), 'BandwidthLimit', 'off'); 
...
[Y, T, YUNIT, XUNIT, HEADER] = invoke(groupObj, 'readwaveform', sprintf('channel%d',ch_no));
